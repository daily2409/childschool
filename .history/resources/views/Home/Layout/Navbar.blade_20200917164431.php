        ...
        <div id="p7PM3tb_1" class="pmm3-toolbar closed"><a href="#" title="Hide/Show Menu">&equiv;</a></div>
            <ul>
                @foreach($menu as $item)
                        @if($item->href)
                            <li><a href="http://localhost/childschool/public/news/{{$item->slug}}" title="{{$item->name}}">{{$item->name}}</a>
                                ...
                                <ul>
                                    @if ($item->menu_child)
                                        @foreach ($item->menu_child as $it)
                                            <li><a href="http://localhost/childschool/public/news".{{$item->slug}}."/".{{$it->slug}} title="{{$it->name}}">{{$it->name}}</a></li>
                                        @endforeach
